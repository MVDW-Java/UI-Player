...
export class AudioPlayer {
  ...
  async init(metadata) {
    if (metadata) {
      ...
      if (this.audioContext.audioWorklet) {
        await this.audioContext.audioWorklet.addModule('lib/nikku/audioMixer.js');
      }
  ...
  initPlayback(bufferStart = 0) {
	...
	if(!this._volume){
		this._volume = document.getElementById('controls-volume').value;
		alert("Report this to MVDW: that one known audio error..\nSetting 'this.__volume' to " + this._volume);
	}
